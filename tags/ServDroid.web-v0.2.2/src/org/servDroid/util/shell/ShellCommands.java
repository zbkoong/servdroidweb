...
public class ShellCommands {
	...
	public static boolean isDeviceRooted() {
		...
		try {
			if (runCommand(new String[] { "su", "-c", "id " }, true)
					.getOutput().contains("uid=0")) {
				Log.d(ServDroid.TAG, "Root access alloed");
				return true;
			}
		} catch (IOException e) {
			return false;
	...
	public static String doSuCmds(List<String> cmds) throws IOException,
		...
		while ((read = reader.read(buffer)) > 0) {
			output.append(buffer, 0, read);
			// Log.d(ServDroid.TAG, " output: "+output.toString());
		}
